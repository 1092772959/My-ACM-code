...
int dfs(int pos, int seven = 0, int mod = 0, bool limit = true)
{
    if (!~pos) return seven || mod == 0;
    if (!limit && ~dp[pos][seven][mod]) return dp[pos][seven][mod];
    int up = limit ? a[pos] : 9;
    int ans = 0;
    for (int i = 0; i <= up; i++) ans += dfs(pos - 1, seven | i == 7, (mod * 10 + i) % 7, limit & i == a[pos]);
